...
IS_HTTPS = config(
    "IS_HTTPS",
    default=not DEBUG,
    help_text="Used to construct absolute URLs and controls a variety of security settings",
)
...
DATABASES = {
    "default": {
        ...
        "HOST": config(
            ...
            "localhost",
            group="Database",
            help_text="hostname of the PostgreSQL database",
        ),
        "PORT": config(
...
subpath = config(
    "SUBPATH", None, help_text="The subpath the application will be mounted on"
)
if subpath:
...
RELEASE = config(
    "RELEASE",
    GIT_SHA,
    help_text="The version number or commit hash of the application (this is also send to Sentry)",
)
...
ELASTIC_APM = {
    ...
    "SERVICE_NAME": config(
        ...
        help_text="Name of the service for this application in Elastic APM",
        group="Elastic APM",
    ),
    "SECRET_TOKEN": config(
